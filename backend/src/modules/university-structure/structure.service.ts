...
type EntityName = "university" | "faculty" | "department" | "program" | "course";

const configs: Record<
  EntityName,
  { model: Model<any>; searchFields: string[]; populate?: Array<{ path: string; select: string }> }
> = {
  university: { model: UniversityModel, searchFields: ["name", "code", "shortName"] },
  faculty: {
    model: FacultyModel,
...
export async function listEntities(entity: EntityName, query: Record<string, unknown>) {
  ...
  if (parentField && query.parentId) filter[parentField] = toObjectId(String(query.parentId), "parent id");

  let findQuery = model.find(filter).sort({ createdAt: -1 }).skip(skip).limit(limit);
  for (const item of populate) findQuery = findQuery.populate(item);
  const [items, total] = await Promise.all([findQuery.lean(), model.countDocuments(filter)]);
...
async function requireActive(model: Model<any>, id: string, label: string) {
  const item = await model.findOne({ _id: toObjectId(id, `${label} id`), status: "active" }).select("_id").lean();
  if (!item) throw new AppError(400, `Active ${label} not found`);
  return item._id as Types.ObjectId;
}

async function ensureUnique(model: Model<any>, filter: Record<string, unknown>, id?: string) {
  const existing = await model.exists(id ? { ...filter, _id: { $ne: toObjectId(id) } } : filter);
  if (existing) throw new AppError(409, "Code already exists within the selected parent");
}
...
export async function createFaculty(input: Record<string, unknown>) {
  const university = await requireActive(UniversityModel, String(input.universityId), "university");
...
export async function createDepartment(input: Record<string, unknown>) {
  const faculty = await requireActive(FacultyModel, String(input.facultyId), "faculty");
...
export async function createProgram(input: Record<string, unknown>) {
  const department = await requireActive(DepartmentModel, String(input.departmentId), "department");
...
async function validatePrerequisites(ids: string[], program: Types.ObjectId, excludedId?: string) {
  const unique = [...new Set(ids)];
  if (excludedId && unique.includes(excludedId)) throw new AppError(400, "A course cannot require itself");
  ...
  if (count !== objectIds.length) {
    throw new AppError(400, "Prerequisites must be active courses in the same program");
  }
  return objectIds;
}

export async function createCourse(input: Record<string, unknown>) {
  const program = await requireActive(ProgramModel, String(input.programId), "program");
  await ensureUnique(CourseModel, { program, code: input.code });
  const prerequisites = await validatePrerequisites((input.prerequisiteIds as string[]) ?? [], program);
  const { programId: _, prerequisiteIds: __, ...data } = input;
  return CourseModel.create({ ...data, program, prerequisites });
}

async function assertCanArchive(entity: EntityName, id: Types.ObjectId): Promise<void> {
  const children: Partial<Record<EntityName, { model: Model<any>; field: string }>> = {
  ...
  if (child && (await child.model.exists({ [child.field]: id, status: "active" }))) {
    throw new AppError(409, `Archive active child records before archiving this ${entity}`);
  }
}

export async function updateEntity(entity: EntityName, id: string, input: Record<string, unknown>) {
  ...
  const parentMap: Partial<Record<EntityName, { input: string; field: string; model: Model<any>; label: string }>> = {
    faculty: { input: "universityId", field: "university", model: UniversityModel, label: "university" },
    department: { input: "facultyId", field: "faculty", model: FacultyModel, label: "faculty" },
    program: { input: "departmentId", field: "department", model: DepartmentModel, label: "department" },
    course: { input: "programId", field: "program", model: ProgramModel, label: "program" },
  };
  const parentConfig = parentMap[entity];
  let parentId: Types.ObjectId | undefined;
  if (parentConfig) {
    parentId = input[parentConfig.input]
      ? await requireActive(parentConfig.model, String(input[parentConfig.input]), parentConfig.label)
      : (current.get(parentConfig.field) as Types.ObjectId);
    if (input.code) await ensureUnique(model, { [parentConfig.field]: parentId, code: input.code }, id);
  ...
  if (entity === "course" && input.prerequisiteIds) {
    current.set(
      "prerequisites",
      await validatePrerequisites(input.prerequisiteIds as string[], parentId!, id),
    );
