...
type CreateFunction = (input: Record<string, unknown>) => Promise<{ _id: unknown }>;
...
export const list = (entity: Entity) => async (req: Request, res: Response): Promise<Response> =>
  sendSuccess(res, 200, `${entity} records retrieved`, await service.listEntities(entity, req.query));

export const getOne = (entity: Entity) => async (req: Request, res: Response): Promise<Response> =>
  sendSuccess(res, 200, `${entity} retrieved`, {
    [entity]: await service.getEntity(entity, req.params.id as string),
  });

export const create = (entity: Entity) => async (req: Request, res: Response): Promise<Response> => {
...
export const update = (entity: Entity) => async (req: Request, res: Response): Promise<Response> => {
...
export const archive = (entity: Entity) => async (req: Request, res: Response): Promise<Response> => {
  const id = req.params.id as string;
