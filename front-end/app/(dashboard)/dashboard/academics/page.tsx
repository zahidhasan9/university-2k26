...
export default async function AcademicsPage() {
  ...
  const totalByEntity = Object.fromEntries(counts) as Record<
    string,
    number | null
  >;
    ...
    <div className="mx-auto max-w-[1400px] space-y-7">
      <div>
        ...
        <p className="mt-1 max-w-2xl text-sm text-muted-foreground">
          Manage the complete institutional hierarchy, curriculum, and academic
          calendar.
        </p>
      </div>
      <div className="rounded-2xl bg-gradient-to-r from-slate-950 via-blue-950 to-blue-800 p-6 text-white sm:p-8">
        ...
        <div className="mt-5 flex flex-wrap items-center gap-2 text-sm font-semibold">
          {["University", "Faculty", "Department", "Program", "Course"].map(
            (item, index) => (
        ...
        <p className="mt-6 max-w-2xl text-sm leading-6 text-slate-300">
          Each level inherits its institutional context, keeping programs and
          courses correctly scoped for enrollment, routines, results, and
          reporting.
        </p>
      </div>
