...
export default async function BatchSectionsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const batch = (await authenticatedRequest<{ batch: Batch }>(API_ENDPOINTS.academics.batchDetail(id))).data.batch
  redirect(`/dashboard/academics/courses?${new URLSearchParams({
