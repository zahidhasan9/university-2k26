...
export default async function NewAcademicRecordPage({ params }: { params: Promise<{ entity: string }> }) {
  ...
  return <AcademicFormShell title={`Create ${config.singular.toLowerCase()}`} description={`Add a new ${config.singular.toLowerCase()} to the academic structure.`} backHref={`/dashboard/academics/${config.key}`}><AcademicForm entity={config.key} options={options} /></AcademicFormShell>
}
