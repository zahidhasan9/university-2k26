...
const modules = [
  {
    key: "employees",
    label: "Employees",
    endpoint: "/hr/employees?limit=1",
    href: "/dashboard/faculty/hr/employees",
    icon: UserCog,
    description: "Employment records and contracts",
  },
  {
    key: "attendance",
    label: "Attendance",
    endpoint: "/hr/attendance",
    href: "/dashboard/faculty/hr/attendance",
    icon: CalendarCheck,
  ...
  },
  {
    ...
    endpoint: "/hr/leaves",
    href: "/dashboard/faculty/hr/leaves",
    icon: ClipboardList,
    description: "Leave review and approval queue",
  },
  {
    key: "payrollRuns",
    label: "Payroll runs",
    endpoint: "/hr/payroll-runs",
    href: "/dashboard/faculty/hr/payroll",
    icon: Banknote,
...
export default async function HrWorkspacePage() {
  ...
  return (
    <div className="mx-auto max-w-6xl space-y-6">
      <Button variant="ghost" render={<Link href="/dashboard/faculty" />}>
        <ArrowLeft /> Faculty directory
      </Button>
      <div>
        <p className="text-sm font-medium text-primary">People operations</p>
        <h1 className="mt-1 text-3xl font-bold tracking-tight">HR workspace</h1>
        <p className="mt-1 text-sm text-muted-foreground">
          Employment, attendance, leave, and payroll operations.
        </p>
      </div>
          ...
          return (
            <Card key={module.key}>
              <CardHeader className="flex-row items-start justify-between">
                ...
                <span className="text-2xl font-bold">{counts[index] ?? "—"}</span>
              </CardHeader>
              <CardContent>
                <CardTitle>{module.label}</CardTitle>
                <p className="mt-2 text-sm text-muted-foreground">{module.description}</p>
                <Button variant="link" className="mt-4 px-0" render={<Link href={module.href} />}>
