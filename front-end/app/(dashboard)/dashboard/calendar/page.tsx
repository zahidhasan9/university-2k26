...
export default async function RoutinePage({ searchParams }: { searchParams: SearchParams }) {
  ...
  if (routineResult.status === "fulfilled") data = routineResult.value.data
    ...
    error =
      routineResult.reason instanceof Error ? routineResult.reason.message : "Routine unavailable"
      ...
      {error ? (
        ...
        <Card>
          <CardContent className="p-0">
              ...
              <TableBody>
                {slots.map((slot) => (
                  <TableRow key={slot._id}>
                    ...
                    <TableCell>
                      {slot.offering.batch ?? "Unassigned"} / {slot.offering.section}
                    </TableCell>
                    <TableCell>
            ...
            {!slots.length && (
              <p className="p-10 text-center text-sm text-muted-foreground">No matching classes</p>
            )}
          </CardContent>
        </Card>
      ) : (
        <div className="grid gap-4 lg:grid-cols-2 xl:grid-cols-3">
          {days
            .filter((day) => !selected.dayOfWeek || day === selected.dayOfWeek)
            .map((day) => {
                        ...
                        <div key={slot._id} className="rounded-xl border p-4">
                          ...
                          <div className="mt-3 grid gap-1 text-xs text-muted-foreground">
                            ...
                            <span className="flex items-center gap-1">
                              ...
                              Batch {slot.offering.batch ?? "Unassigned"}, Section{" "}
                              {slot.offering.section}
                            </span>
                            <span>
                              {slot.teacher.user.firstName} {slot.teacher.user.lastName}
                            </span>
