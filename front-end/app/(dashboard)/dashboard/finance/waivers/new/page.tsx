...
export default async function NewWaiverPage({
  ...
  if (studentId)
    try {
      student = (
        await authenticatedRequest<{ student: Student }>(API_ENDPOINTS.students.detail(studentId))
      ).data.student
    } catch {
      student = undefined
    }
  return (
    <FinanceFormShell
      title="Assign student waiver"
      description="Create a dated tuition or full-invoice scholarship for a student."
    >
      <WaiverForm initialStudent={student} />
    </FinanceFormShell>
  )
}
