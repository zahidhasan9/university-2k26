...
type Notice = {
  ...
  publishAt: string
  expiresAt?: string
  audienceRoles: { _id: string; code: string; name: string }[]
  createdBy: Person
}
type Conversation = {
  _id: string
...
export default async function CommunicationPage() {
  ...
  try {
    ...
    notifications = data[2].data.items
  } catch (cause) {
    error = cause instanceof Error ? cause.message : "Communication data unavailable"
  }
  ...
  return (
    <div className="mx-auto max-w-[1600px] space-y-6">
      <div className="flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
        <div>
          <p className="text-sm font-medium text-primary">Connected campus</p>
          <h1 className="mt-1 text-3xl font-bold">Communication</h1>
          <p className="mt-1 text-sm text-muted-foreground">
            Notices, conversations, and delivery notifications in one workspace.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Button variant="outline" render={<Link href="/dashboard/communication/notices/new" />}>
            <Megaphone />
            New notice
          </Button>
          <Button render={<Link href="/dashboard/communication/conversations/new" />}>
            <Plus />
            New conversation
          </Button>
          <Button
            variant="outline"
            render={<Link href="/dashboard/communication/notifications/new" />}
          >
            <Mail />
            Dispatch
          </Button>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        <Metric icon={<Megaphone />} value={notices.length} label="Visible notices" />
        <Metric icon={<MessageCircle />} value={conversations.length} label="Conversations" />
        <Metric icon={<Bell />} value={unread} label="Unread notifications" />
      </div>
      <div className="grid gap-6 xl:grid-cols-[1.15fr_.85fr]">
        <Card>
          ...
          <CardContent className="space-y-4">
            {notices.length === 0 && <Empty text="No notices are available." />}
            {notices.map((item) => (
                ...
                <div className="flex flex-wrap items-start justify-between gap-2">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      {new Date(item.publishAt).toLocaleString()} · {item.createdBy?.firstName}{" "}
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            {conversations.map((item) => (
              <Link
                href={`/dashboard/communication/conversations/${item._id}`}
                key={item._id}
              ...
              >
                <div className="flex justify-between gap-3">
                  <div>
                    ...
                    <p className="mt-1 text-xs text-muted-foreground">
                      {item.participants.length} participants · {item.type}
                    </p>
                  </div>
        ...
        <CardContent className="space-y-3">
          ...
          {notifications.map((item) => (
            ...
            >
              ...
              {item.status !== "read" && <ReadNotification id={item._id} />}
            </div>
          ))}
        </CardContent>
      </Card>
