...
export default async function ExamMarksPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const enrollments = (
    await authenticatedRequest<{ items: { student: Student }[] }>(
      `/enrollments?offeringId=${exam.offering._id}&status=enrolled&limit=100`,
    )
  ).data.items
  return (
    <div className="mx-auto max-w-5xl space-y-6">
      ...
      <div>
        ...
        <p className="mt-1 text-sm text-muted-foreground">
          {exam.offering.course.code} · Section {exam.offering.section} · {exam.totalMarks} marks
        </p>
      </div>
      <Card>
        <CardHeader>
        ...
        <CardContent>
          <MarksEntry
            ...
            students={enrollments.map((item) => item.student)}
            existing={marksResponse.data.marks}
          />
        </CardContent>
      </Card>
    </div>
