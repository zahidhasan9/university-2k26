...
export function ManualAttendance({
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const records = students.map((student) => ({
      ...
      status: String(form.get(`status-${student._id}`)),
  ...
  return (
    <form onSubmit={submit} className="rounded-xl border">
      <div className="flex items-center justify-between border-b p-4">
        <div>
          <p className="font-semibold">Manual attendance</p>
          <p className="text-xs text-muted-foreground">{students.length} enrolled students</p>
        </div>
        <Button type="submit" disabled={loading || !students.length}>
          {loading ? <LoaderCircle className="animate-spin" /> : <Save />}Save attendance
        </Button>
      </div>
      <div className="max-h-[520px] divide-y overflow-y-auto">
        {students.map((student) => {
          const record = previous.get(student._id)
          return (
            <div
              key={student._id}
              className="grid gap-3 p-4 sm:grid-cols-[1fr_150px_220px] sm:items-center"
            >
              <div>
