  ...
  return (
    <div>
      <div className="flex flex-wrap gap-2">
        <Button variant="outline" disabled={Boolean(loading)} onClick={() => run("calculate")}>
          {loading === "calculate" ? <LoaderCircle className="animate-spin" /> : <Calculator />}
          Calculate results
        </Button>
        <Button disabled={Boolean(loading) || !hasDraft} onClick={() => run("publish")}>
          {loading === "publish" ? <LoaderCircle className="animate-spin" /> : <Send />}Publish
          results
        </Button>
      </div>
