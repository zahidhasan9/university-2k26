...
export type AcademicSectionOption = { _id: string; code: string; name: string; capacity: number; enrolledCount: number; availableSeats: number; shift: string; homeRoom?: string; status: string }
...
export function AcademicSectionManager({ batchId, sections }: { batchId: string; sections: AcademicSectionOption[] }) {
  ...
  async function archive(id: string) {
    ...
    if (!response.ok) { const body = await response.json<{ message?: string }>(); throw new Error(body.message || "Section could not be archived") }
    router.refresh()
  }
  return <div className="space-y-6">
    <form onSubmit={create} className="grid gap-4 rounded-xl border bg-muted/20 p-5 md:grid-cols-5">
      <div className="space-y-2"><Label>Code</Label><Input name="code" placeholder="A" required /></div>
      <div className="space-y-2"><Label>Name</Label><Input name="name" placeholder="Section A" required /></div>
      <div className="space-y-2"><Label>Capacity</Label><Input name="capacity" type="number" min={1} max={1000} defaultValue={60} required /></div>
    ...
    <div className="overflow-hidden rounded-xl border">
      ...
      <tbody>{sections.map((section) => <tr key={section._id} className="border-t"><td className="p-3"><p className="font-semibold">{section.code}</p><p className="text-xs text-muted-foreground">{section.name}</p></td><td className="p-3 capitalize">{section.shift}{section.homeRoom ? ` · ${section.homeRoom}` : ""}</td><td className="p-3"><span className="inline-flex items-center gap-1"><Users className="size-4" />{section.enrolledCount} / {section.capacity}</span></td><td className="p-3">{section.availableSeats}</td><td className="p-3 text-right"><ConfirmAction title={`Archive section ${section.code}?`} description="A section with assigned students cannot be archived. Transfer them first." confirmLabel="Archive section" triggerIcon={<Trash2 />} onConfirm={() => archive(section._id)} disabled={section.enrolledCount > 0} /></td></tr>)}</tbody></table>
      {!sections.length && <p className="p-8 text-center text-sm text-muted-foreground">No sections yet. Add the first section above.</p>}
    </div>
  </div>
}
